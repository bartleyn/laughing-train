...
'''
def local_bias(model, length, attn_func = None):
    #self.edges_seen.add((tweet[0], user, float(in_deg(user)), float(self.user_map[user])))

    edges = np.array(list(model['edges_seen'][length]))#np.array([[int(x) for x in edge.split('_')] for i, edge in enumerate(model.edges_seen)])
    #np.array([(friend, user) for friend in model.edges_seen for user in model.edges_seen[friend]])

    user_map = model['user_map']   # nede to actually use this 
    mean_in_deg = np.nanmean(edges[:,2])
    true_prev = model['true_prevalence']
    ...
    vals[:,3] = np.vectorize(user_map.get)(vals[:,1])#list(map(user_map, vals[:,1]
    #print("edges: ", edges)
    #vectorize
    try:
...
def compute_gini(array, ):
    ...
    array = np.ravel(array.flatten())
    #print("arr shape {}".format(array.shape))
    #print("ravel arr shape {}".format(np.ravel(array).shape))
    #print("arr squeezed shape {}".format(np.squeeze(array).shape))
    try:
        if np.amin(array) < 0:
    ...
    n = array.shape[0]
    # Gini coefficient:
    #print("n {} shape {} index shape {} sum_array {} computation {}".format(n, array.shape, index.shape, np.sum(array), ((np.sum((2 * index - n  - 1) * array)) / (n * np.sum(array)))))
    if n == 0:
        return np.nan
    else:
...
def get_correlation_graph(corr_dicts, users, start, end, kind, corrs, val, get_ct_majority=True):
    ...
    elif val == 1 and get_ct_majority == False:
        title = 'Frac. (+) Users Per Day'
        ylabel = 'Frac. (+) Users Per Day'
    elif val == 1 and get_ct_majority == True:
        title = 'Avg. Users with MI Per Day'
        ylabel = 'Avg. Users with MI Per Day'
        ...
        for corr in corrs:#, 0.75, 1.0]:
            ...
            for user in users:
                ...
                if len(vals_to_plot) == 0:
                    continue
                if val == 1 and get_ct_majority:
                    nums.append(sum([1 if x > 0.5 else 0 for x in vals_to_plot if not np.isnan(x) ])/len(vals_to_plot) )  ## TODO FINISH THIS 
